...
#define ASSERT_RETV(call_expr, condition_expr ) \
    if (!((call_expr) condition_expr)){    \
        unix_error(#call_expr " error");   \
    }
...
void listbgjobs(struct job_t* job);
    ...
    if (!builtin_cmd(argv)) {
        ...
        if (pid == 0) {
            ...
            if (!bg) {
                ...
                sigprocmask(SIG_UNBLOCK, &sigmask, NULL);   //在添加job后解除对SIGCHLD的屏蔽
                waitfg(pid);
            } else {
...
void do_bgfg(char** argv)
{
    ...
    if (!strcmp(argv[0], "bg")) {
        ...
        //如果不等于"bg"则一定等于"fg",在buildin_cmd中已经判断过
        debug("switching job [%d] (%d) state to foreground\n", job->jid, pid);
        kill(-pid, SIGCONT);
        job->state = FG;
        waitfg(pid);
    }
...
void sigchld_handler(int sig)
{
    ...
    while ((pid = waitpid(-1, &status, WNOHANG | WUNTRACED | WCONTINUED)) > 0) {
        ...
        if (WIFEXITED(status) || WIFSIGNALED(status)) {
            ...
            //jobstate在此处设置,为应对子进程不通过bg自行后台启动的情况
...
void sigint_handler(int sig)
{
    ...
    debug("signal %d catched. Foreground job:%d\n", sig, pid);
    if (pid != 0) {
        kill(-pid, sig); //转发信号
        //中断的一定是前台程序,而父进程一定会调用waitfg,故统一在waitfg中删除joblist中对应的条目
    }
}
